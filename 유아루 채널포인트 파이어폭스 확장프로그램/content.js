  ...
  return new Promise((resolve, reject) => {
    browser.runtime.sendMessage(message, (response) => {
      if (browser.runtime.lastError) {
        console.error("🚨 메시지 전송 오류:", browser.runtime.lastError);
...
function addButton() {
  ...
  waitForElement("aside-chatting", (targetElement) => {
    ...
    browser.runtime.sendMessage({ type: "start_websockets", streamerUUID: broadcastUid });
  });
}
  ...
  Object.entries(rewards).forEach(([rewardName, rewardPoints]) => {
    ...
    rewardButton.addEventListener("click", async () => {
      ...
      if (await isBan()) {
        ...
        // 입력값과 함께 보상 요청 보내기
        browser.runtime.sendMessage({ type: "redeem_reward", streamerUUID: broadcastUid, rewardPoints: rewardPoints, inputValue: inputValue });
